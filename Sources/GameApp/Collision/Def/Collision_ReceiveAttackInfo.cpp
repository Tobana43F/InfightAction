#include	"Collision_ReceiveAttackInfo.h"

#include	"Collision\Holder\TypeData\Collision_TypeDataAttack.h"
#include	"Collision\Holder\TypeData\Collision_TypeDataDamage.h"
#include	"Object\Object_GameObject.h"

namespace NCollision {

	CReceiveAttackInfo::CReceiveAttackInfo() :
		m_isEnable(false),
		m_attackSequenceID(UINT_MAX),	// 無効な値を代入
		m_attackGroupID(UINT_MAX),
		m_attackSerialID(UINT_MAX)
	{
	...
	void CReceiveAttackInfo::SetData(
	...
	bool CReceiveAttackInfo::CheckIsSameID(const CReceiveAttackInfo& _rhs) const
	{
		// 有効ではない場合、判断しない
		if (!m_isEnable) { return false; }
		if (!_rhs.m_isEnable) { return false; }

		// どれか一つでも違えば別のものだと判断する
		bool sts = true;
		sts &= (m_pOwner == _rhs.m_pOwner);	// ポインタの指す場所をIDとして比較
		sts &= (m_attackSequenceID == _rhs.m_attackSequenceID);
		sts &= (m_attackGroupID == _rhs.m_attackGroupID);
		sts &= (m_attackSerialID == _rhs.m_attackSerialID);
		return sts;
	}
}